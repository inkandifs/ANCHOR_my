...
function MasterPage({ kind }: { kind: 'contacts' | 'products' | 'analytics' | 'accounts' | 'journals' }) {
  ...
  const items = {
    ...
    accounts,
    journals,
    analytics: budgets.map((b: any) => ({ ...b, name: b.analytic, type: b.type, period: b.period, achieved: b.achieved, sales: b.achieved, target: b.target }))
  }[kind] || [];

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (kind === 'contacts') {
      await addContact({ name: nameInput || 'New Contact', type: typeInput || 'Customer', email: descInput || 'contact@domain.com', city: 'Workspace HQ' });
    } else if (kind === 'products') {
      await addProduct({ name: nameInput || 'New Product', category: typeInput || 'Goods', unitPrice: '120.00', costPrice: '60.00' });
    } else if (kind === 'accounts') {
      await addAccount({ name: nameInput || 'New Account', type: typeInput || 'Asset', balance: '0.00' });
    } else if (kind === 'journals') {
      await addJournal({ name: nameInput || 'New Journal', type: typeInput || 'General' });
    }
    setModal(false);
    setNameInput('');
    setDescInput('');
  ...
  const handleDeleteItem = async (id: any) => {
    if (kind === 'contacts') await deleteContact(id);
    else if (kind === 'products') await deleteProduct(id);
    else if (kind === 'accounts') await deleteAccount(id);
    else if (kind === 'journals') await deleteJournal(id);
  };
...
function TransactionList({ type }: { type: 'purchase' | 'sales' | 'journal' }) {
  const [, setLocation] = useLocation();
  const [view, setView] = useState<'list' | 'kanban'>('list');
  const { purchaseOrders, deletePurchaseOrder } = usePurchaseOrders();
  const { salesOrders, deleteSalesOrder } = useSalesOrders();
  const { journalEntries, deleteJournalEntry } = useJournalEntries();
  ...
  const rows = type === 'purchase' ? purchaseOrders : type === 'sales' ? salesOrders : journalEntries;

  const handleDeleteRow = async (id: any) => {
    if (type === 'purchase') await deletePurchaseOrder(id);
    else if (type === 'sales') await deleteSalesOrder(id);
    else if (type === 'journal') await deleteJournalEntry(id);
  };
...
function DetailForm({ mode }: { mode: 'purchase' | 'sales' | 'journal' }) {
  ...
  const [lineItems, setLineItems] = useState<any[]>([
    { id: 1, productName: 'Brand strategy sprint', analyticTag: 'Growth', qty: 1, unitPrice: 4800, lineTotal: 4800 },
    { id: 2, productName: 'Design system retainer', analyticTag: 'Operations', qty: 1, unitPrice: 2400, lineTotal: 2400 }
  ]);
  ...
  const targetType = isPurchase ? 'Vendor' : 'Customer';
  const matchingContacts = contacts.filter(c => !c.type || c.type === targetType || c.type === 'Both');
  ...
  const productOptions = products.map(p => {
    const priceNum = parseFloat((p.sales || p.unitPrice || '0').replace(/[^0-9.]/g, '')) || 0;
    return {
      label: p.name,
      value: p.name,
      sub: p.sales || `$${priceNum.toFixed(2)}`,
      data: { price: priceNum, name: p.name }
    };
  });

  const computedTotalNum = lineItems.reduce((acc, item) => acc + (Number(item.lineTotal) || 0), 0);
  const formattedTotal = `$${computedTotalNum.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;

  const handleProductSelect = (index: number, val: string, selectedData?: any) => {
    const updated = [...lineItems];
    const unitPrice = selectedData?.price ?? updated[index].unitPrice;
    updated[index] = {
      ...updated[index],
      productName: val,
      unitPrice,
  ...
  const handleUnitPriceChange = (index: number, newPrice: number) => {
    const updated = [...lineItems];
    const unitPrice = Math.max(0, newPrice || 0);
    updated[index] = {
      ...updated[index],
      unitPrice,
      lineTotal: updated[index].qty * unitPrice
  ...
  const handleAddLine = () => {
    setLineItems([
      ...lineItems,
      { id: Date.now(), productName: '', analyticTag: 'General', qty: 1, unitPrice: 0, lineTotal: 0 }
    ]);
  };
  ...
  const handleCreate = async () => {
    if (!isJournal && !partner.trim()) {
      alert(`Please select a ${isPurchase ? 'Vendor' : 'Customer'} before confirming.`);
      return;
    }
  ...
  return (
    <>
      <Breadcrumb section={isJournal ? 'Account' : isPurchase ? 'Purchase' : 'Sales'} page={`New ${label}`} />
      <PageTitle
        eyebrow={`${mode.toUpperCase()} / NEW`}
        title={`New ${label}`}
        detail={isJournal ? 'Record a precise, balanced movement in the books.' : 'Build the next record from a trusted starting point.'}
        action={
          <div className="stage-actions">
            <StatusPill status={step} />
            <Button variant="ghost" onClick={handleCancel}>Cancel</Button>
            <Button onClick={handleCreate}>{isJournal ? 'Post entry' : 'Confirm'} <ArrowRight size={15} /></Button>
          </div>
        }
      />
        ...
        <section className="form-card">
          <div className="card-section-head">
            <div>
              ...
              <h3>{isJournal ? 'Entry information' : `${label} information`}</h3>
            </div>
            <span className="auto-number">{isJournal ? 'JE-2026-019' : isPurchase ? 'PO-2026-043' : 'SO-2026-039'}</span>
          </div>
          <div className="form-grid">
            {isJournal ? (
              <Field label="Accounting date" value={dateVal} onChange={setDateVal} placeholder="Mar 24, 2026" />
            ) : (
              <div className="field">
                <Combobox
                  label={isPurchase ? 'Vendor' : 'Customer'}
            ...
            <Field
              label={isJournal ? 'Journal' : 'Date'}
              value={isJournal ? journalVal : dateVal}
              onChange={isJournal ? setJournalVal : setDateVal}
              placeholder={isJournal ? 'General Journal' : 'Mar 24, 2026'}
            />

            {isJournal ? (
              <Field label="Reference / memo" value={refVal} onChange={setRefVal} placeholder="Short description" />
            ) : (
              <Field label="Total amount" value={formattedTotal} onChange={() => {}} placeholder="0.00" helper="Calculated live from line items" />
            )}
          </div>
        </section>
        ...
        {!isJournal && (
          <div className="amber-banner">
            ...
            <span>
              <b>Budget watch</b>
              <br />
              This commitment will use 8.4% of the linked Operations budget.
            </span>
            <button type="button">Review budget</button>
          </div>
        )}

        <section className="form-card line-items">
          <div className="card-section-head">
            <div>
              <span className="eyebrow">LINE ITEMS</span>
              <h3>{isJournal ? 'Debit & credit lines' : 'Products and services'}</h3>
            </div>
          ...
          <table>
            <thead>
              <tr>
                {isJournal
                  ? ['Account / Item', 'Analytic tag', 'Debit', 'Credit', '']
                  : ['Product / Service', 'Analytic tag', 'Qty', 'Unit price', 'Line total', '']}
              </tr>
            </thead>
            <tbody>
                  ...
                  <td>
                    <div style={{ minWidth: '220px' }}>
                      <Combobox
                        ...
                        onChange={(val, data) => handleProductSelect(idx, val, data)}
                        testId={`input-line-product-${idx}`}
                      />
                    </div>
                  </td>
                  <td>
                    <input
                      type="text"
                      value={item.analyticTag || 'General'}
                      onChange={e => {
                        const updated = [...lineItems];
                        updated[idx].analyticTag = e.target.value;
                        setLineItems(updated);
                      }}
                      style={{ height: '38px', padding: '0 8px', width: '100%', borderRadius: '4px', border: '1px solid var(--line)', background: 'transparent' }}
                    />
                  </td>
                  <td>
                    <input
                      ...
                      style={{ height: '38px', padding: '0 8px', width: '70px', borderRadius: '4px', border: '1px solid var(--line)', background: 'transparent' }}
                    />
                  </td>
                  <td>
                    <input
                      ...
                      style={{ height: '38px', padding: '0 8px', width: '100px', borderRadius: '4px', border: '1px solid var(--line)', background: 'transparent' }}
                    />
                  </td>
                  <td className="money" style={{ verticalAlign: 'middle', fontWeight: 600 }}>
                  ...
                  <td style={{ verticalAlign: 'middle', width: '36px' }}>
                    {lineItems.length > 1 && (
                      <button
                        ...
                        style={{ border: 'none', background: 'transparent', color: '#943737', cursor: 'pointer', padding: '4px' }}
                        title="Remove line"
                      >
                        <Trash2 size={15} />
                      </button>
            ...
            <tfoot>
              <tr>
                <td colSpan={isJournal ? 2 : 4}>
                  <b>{isJournal ? (balanced ? 'Balanced and ready to post' : 'Totals do not match') : `${lineItems.length} line item${lineItems.length > 1 ? 's' : ''}`}</b>
                </td>
                <td className={isJournal && !balanced ? 'danger-text' : 'money'} style={{ fontWeight: 700, fontSize: '13px' }}>
                  {formattedTotal}
                </td>
                <td />
              </tr>
          ...
          {isJournal && (
            <div className={`balance-toggle ${balanced ? 'balanced' : ''}`}>
              <span>{balanced ? <Check size={15} /> : <X size={15} />} Visual balance state</span>
              <button type="button" onClick={() => setBalanced(!balanced)} data-testid="button-toggle-balance">
                {balanced ? 'Show mismatch' : 'Mark balanced'}
              </button>
            </div>
          )}
        </section>
...
function BillingPage({ mode }: { mode: 'bill' | 'invoice' }) {
  const { vendorBills, addVendorBill } = useVendorBills();
  const { customerInvoices, addCustomerInvoice } = useCustomerInvoices();
...
function Portal({ role, setRole }: { role?: Role; setRole?: (r: Role) => void }) {
  const { customerInvoices } = useCustomerInvoices();
  const [rows, setRows] = useState<any[]>([]);
  useEffect(() => { setRows(customerInvoices); }, [customerInvoices]);
...
function App() { return <QueryClientProvider client={queryClient}><TooltipProvider><WouterRouter base={import.meta.env.BASE_URL.replace(/\/$/, '')}><ErrorBoundary><StoreProvider><RoutedApp /></StoreProvider></ErrorBoundary></WouterRouter><Toaster /></TooltipProvider></QueryClientProvider>; }
export default App;